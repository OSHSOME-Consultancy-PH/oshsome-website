...
export default function Module3aPage() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await axios.post(`${API}/so3b-requests`, {
        ...data,
        source_page: "so3b",
      });
      ...
      const ok = saveFormLocally("osh-so3b-requests", data);
    ...
    <div style={{ fontFamily: brand.fonts.body }}>
      ...
      <section className="relative">
        ...
        <div className="mx-auto max-w-screen-xl px-4 pt-20 pb-16 text-white">
          <h1 className="text-4xl md:text-5xl font-bold" style={{ fontFamily: brand.fonts.heading }}>{so3bContent.hero.title}</h1>
          <p className="mt-3 text-white/90 max-w-2xl">{so3bContent.hero.subtitle}</p>
          <Button className="mt-6" style={{ backgroundColor: brand.colors.gold, color: brand.colors.ink }}>{so3bContent.hero.cta}</Button>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-screen-xl px-4 py-12">
        <Card>
          <CardContent className="pt-6">
            <p className="text-sm leading-relaxed">{so3bContent.overview}</p>
          </CardContent>
        </Card>
      </section>
      ...
      <section className="mx-auto max-w-screen-xl px-4 py-4">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {so3bContent.keyTopics.map((topic) => (
      ...
      <section className="mx-auto max-w-screen-xl px-4 py-10">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {so3bContent.highlights.map((point, idx) => (
      ...
      <section className="mx-auto max-w-screen-xl px-4 py-10">
        ...
        <Accordion type="single" collapsible className="w-full">
          {so3bContent.faq.map((f, idx) => (
            <AccordionItem key={idx} value={`so3b-${idx}`}>
